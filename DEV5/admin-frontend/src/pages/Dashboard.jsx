...
export default function Dashboard() {
  ...
  const loadFilteredData = async () => {
    try {
      setError(null);
      const [eventsData, entriesData] = await Promise.all([
        getEvents(filters),
        getEntries(filters)
      ]);
        ...
        <div style={{ marginBottom: '30px' }}>
          ...
          <div style={{ 
            ...
            <div style={{ 
              ...
              <div style={{ maxHeight: '150px', overflowY: 'auto' }}>
                {Object.entries(stats.eventsPerDay || {}).slice(0, 7).map(([date, count]) => (
                  <div key={date} style={{ display: 'flex', justifyContent: 'space-between', margin: '5px 0' }}>
              ...
              <tbody>
                {entries.map((entry, index) => (
                  <tr key={index}>
                    ...
                    <td style={{ padding: '12px', borderBottom: '1px solid #eee' }}>
                      {entry.zodiacSign || 'N/A'}
                    </td>
                    <td style={{ padding: '12px', borderBottom: '1px solid #eee' }}>
                      {entry.analysis?.sentiment || 'N/A'}
                    </td>
                    <td style={{ padding: '12px', borderBottom: '1px solid #eee' }}>
                      {entry.analysis?.sentimentScore !== undefined ? entry.analysis.sentimentScore.toFixed(2) : 'N/A'}
                    </td>
                    <td style={{ padding: '12px', borderBottom: '1px solid #eee' }}>
                      {entry.userId || 'N/A'}
                    </td>
